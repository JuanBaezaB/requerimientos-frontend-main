...
@Component({
  ...
  templateUrl: './confirm-retry-alert.component.html',
  styleUrl: './confirm-retry-alert.component.scss',
})
export class ConfirmRetryAlertDialogComponent {
  readonly dialogStore = inject(DialogConfirmStore);
  readonly respuestaStore = inject(RespuestaStore);
  private dialogRef = inject(MatDialogRef<ConfirmRetryAlertDialogComponent>);
